...
public:
    ...
    //! Устанавливает JavaVM
    /*!
       Сохраняет указатель на java-машину в статическую глобальную переменную для дальнейшего использования.
       Java-обертки ДТО самостоятельно вызывают данный метод
       \param vm Указатель на java-машину
       \retval -1 Ошибка
       \retval 0 Успех
     */
    virtual int DTOSHARED_CCA put_JavaVM(void *vm) = 0;
    ...
    /*!
       \warning На данный момент возвращает только ошибку EC_CONNECTION_LOST в случае потери связи с ККМ (дККМ) и некоторыми платежными терминалами (дПС).
       \param value Дескриптор
       \retval -1 Ошибка
       \retval 0 Успех
     */
